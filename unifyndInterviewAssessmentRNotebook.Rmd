...
# Number of users who have earned points more than 2000
...
customer_scans_raw_Q3_subset <- customer_scans_raw %>%
  ...
  dplyr::select(customerId, storeName, createdAt) %>%
  dplyr::group_by(storeName) %>%
  dplyr::summarise(numBillsScanned = n()) %>%
...
id "83" scanned bills in February, 2020. It also lists the total number of bills
scanned by customer Id "83" for each store. This information is useful in case
more than one bill is scanned per store. 
...
coupon_transactions_raw_Q4_subset <- coupon_transactions_raw %>%
  ...
  group_by(customerId) %>%
  summarise(numCouponsUnlocked = n()) %>%
...
the number of coupons unlocked by each one of them on September 10, 2021.
...
pivot_table_by_tier_Q5 <- customer_user_coupon_join_Q5 %>%
